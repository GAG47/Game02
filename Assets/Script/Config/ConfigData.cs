...
{
    private Dictionary<int, Dictionary<string, string>> datas; //每个数据表存储的数据到字典中
                                                               //key是字典的id 值是每一行数据
    ...
    public void Load(string txt)
    {
        ...
        string[] titleArr = dataArr[0].Trim().Split(','); //逗号切割 //获得第一行数据 作为每行数据中字典的key值
        //内容和从第三行开始读取（下标从2开始）
    ...
    public Dictionary<string,string> GetDataBy(int id)
    {
